...
class UserMarketPlaceModel(models.Model):
    ...
    marketType = models.CharField(
        verbose_name="Market Tipi", max_length=255, choices=MARKET_TYPE)
    supplierId = models.CharField(verbose_name="Trendyol Satıcı ID / Hepsiburada Merchant ID",
                                  max_length=255, unique=True, blank=True, null=True)
    apiKey = models.CharField(
        verbose_name="Trendyol / N11 Satıcı ApiKey", max_length=255, blank=True, null=True)
    apiSecret = models.CharField(
        verbose_name="Trendyol / N11 Satıcı ApiSecret", max_length=255, blank=True, null=True)
...
class UserMarketShipmentRuleModel(models.Model):
    userMarket = models.ForeignKey(
        UserMarketPlaceModel, on_delete=models.CASCADE)
    ...
    cargo = models.CharField(verbose_name="Kargo Firması",
                             max_length=255, choices=CARGO_CHOICES, blank=True, null=True)
...
class MarketProductModel(models.Model):
    ...
    def save(self, *args, **kwargs):
        if int(self.availableStock) > 0:
            ...
            self.onSale = False
        super(MarketProductModel, self).save(
...
class MarketProductCommissionModel(models.Model):
    mpm = models.ForeignKey(
        MarketProductModel, verbose_name="Ürün", on_delete=models.CASCADE)
    startDate = models.DateTimeField(
        verbose_name="Başlangıç Tarihi", blank=True, null=True)
    endDate = models.DateTimeField(
        verbose_name="Bitiş Tarihi", blank=True, null=True)
...
class MarketMedProductModel(models.Model):
    ...
    mpm = models.ForeignKey(
        MarketProductModel, verbose_name="Pazaryeri Ürünü", on_delete=models.CASCADE)
...
class MarketUpdateQueueModel(models.Model):
    ...
    isUpdated = models.BooleanField(
        verbose_name="Güncellendi mi?", default=False)
...
class MarketProductBuyBoxListModel(models.Model):
    ...
    dispatchTime = models.IntegerField(
        "Kargoya Verme Süresi", blank=True, null=True)
    isCompeted = models.BooleanField(
        verbose_name="Rekabet edildi mi?", default=False)
...
class MarketBuyBoxTraceModel(models.Model):
    marketProduct = models.ForeignKey(
        MarketProductModel, on_delete=models.CASCADE)
    ...
    giveMax = models.BooleanField(
        verbose_name="Rakip yoksa veya rekabet edilemiyorsa max(true)/min(false) fiyatı ver?", default=False)
    recoMax = models.BooleanField(
        verbose_name="Max fiyatı tanı(true)/ma(false)?", default=False)
    isActive = models.BooleanField(verbose_name="Aktif mi?")
...
class MarketOrderModel(models.Model):
    userMarket = models.ForeignKey(
        UserMarketPlaceModel, verbose_name="Pazar Yeri", on_delete=models.CASCADE, blank=True, null=True)
    customerModel = models.ForeignKey(
        "billing.CustomerModel", blank=True, null=True, on_delete=models.CASCADE)
    orderNumber = models.CharField(verbose_name="Sipariş No", max_length=100)
    packageNumber = models.CharField(
        verbose_name="Paket Numarası", max_length=100, blank=True, null=True)
    orderDate = models.DateTimeField(verbose_name="Sipariş Tarihi")
    deliveryDate = models.DateTimeField(
        verbose_name="Teslim Tarihi", blank=True, null=True)
    totalPrice = models.FloatField(
        verbose_name="Toplam Tutar", blank=True, null=True)
    priceToBilling = models.FloatField(
        verbose_name="Faturalandırılacak Tutar", blank=True, null=True)
...
class MarketOrderDetailModel(models.Model):
    mom = models.ForeignKey(
        MarketOrderModel, verbose_name="Sipariş Modeli", on_delete=models.CASCADE)
    mpm = models.ForeignKey(
        MarketProductModel, verbose_name="Market Ürünü", on_delete=models.CASCADE)
...
class MarketOrderPredCostModel(models.Model):
    mom = models.ForeignKey(
        MarketOrderModel, verbose_name="Sipariş Modeli", on_delete=models.CASCADE)
    modm = models.ForeignKey(MarketOrderDetailModel, verbose_name="Sipariş Detay Modeli",
                             on_delete=models.CASCADE, blank=True, null=True)
    costType = models.CharField(
        verbose_name="Gider Türü", max_length=255, choices=COST_TYPES)
